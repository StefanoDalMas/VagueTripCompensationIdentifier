...
def route_similarity(stdRoute: StdRoute, actRoute: ActRoute, is_perfect_route: bool = False) -> float:
    similarity = 0.0
    penality = 0.0
...
def calc_mean_sim(sim_drivers_routes: params.driverSimilarities) -> float:
    drivers_sim = 0
    for driver, routes in sim_drivers_routes.items():
        routes_sim = 0
        for route, sim in routes.items():
            routes_sim += sim
